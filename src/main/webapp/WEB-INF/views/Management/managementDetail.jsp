...
<div class="container">
		<c:choose>
			<c:when test="${emp.role_code eq '사장'}">
...
<script>
	...
	function saveEmployee() {
	    ...
	    var photoChanged = fileInput.files.length > 0; // 사용자가 이미지를 변경했는지 여부 확인
	    ...
	    if (photoChanged) {
	        ...
	        $.ajax({
	            ...
	            success: function(response) {
	                if (response.success) {
	                    // 서버에 저장된 이미지 URL을 폼에 추가
	                    $('#photoUrl').val(response.url);
	                    console.log("Uploaded Photo URL: " + response.url);
