	...
	$(document).ready( function() {
		...
		$.ajax({
	        ...
	        success: function(data) {
	            ...
	            data.forEach(function(i) {
	                labels.push(i.NAME);
	                apprCounts.push(i.APPROVED_COUNT);
	                returnCounts.push(i.RETURN_COUNT);
	            });	 
	            
	            const ctx = $('#docuStatusChart');
	            const docuStatusChart = new Chart(ctx, {
	                type: 'bar',
	                data: {
	                    labels: ['업무 기안', '휴가 신청서', '지각 사유서'],
	                    datasets: [
	                        {
	                            label: '반려',
	                            data: returnCounts, 
		...
		$.ajax({
	        ...
	        success: function(data) {
	            ...
	            data.forEach(function(i) {
	                labels.push(i.ROLE_TITLE);
	                totalCounts.push(i.TOTAL_COUNT);
	                femaleCounts.push(i.FEMALE_COUNT);
	                maleCounts.push(i.MALE_COUNT);
	            });	 
	            
	    		const ctx = $('#empRoleChart');
	    	    const empRoleChart = new Chart(ctx, {
	    	        data: {
	    	        	    labels: labels, 
	    	        	    datasets: [
	    	        	        {
